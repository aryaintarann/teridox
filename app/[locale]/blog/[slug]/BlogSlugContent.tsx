...
function splitFaq(raw: string): { body: string; faqItems: FaqItem[] } {
  ...
  const faqMatch = normalized.match(/^#{1,3}\s*FAQ\s*$/im)
  if (!faqMatch || faqMatch.index === undefined) return { body: normalized, faqItems: [] }

  const body = normalized.slice(0, faqMatch.index).trim()
  const faqSection = normalized.slice(faqMatch.index + faqMatch[0].length).trim()
